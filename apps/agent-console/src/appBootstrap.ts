...
export function buildLocalNodeModelDraftFromStatus(status: LocalNodeStatusResponse | null): LocalNodeModelConfigRequest {
  const openaiApiKeyConfigured = Boolean(status?.model_settings?.openai_api_key_configured);
  const difyApiKeyConfigured = Boolean(status?.model_settings?.dify_api_key_configured);
  ...
  return {
    ...
    preserve_openai_api_key: openaiApiKeyConfigured,
    ...
    preserve_dify_api_key: difyApiKeyConfigured,
