...
export type SessionRecord = { session_id: string; channel: string; user_id: string; agent_id: string; status: SessionStatus; assigned_node_id: string | null; assigned_slot_id: string | null; active_task_id: string | null; queue_status: QueueStatus; context_summary: string; context_version: number; routing_mode: RoutingMode; slot_bound_at: string | null; slot_expires_at: string | null; reply_context_token: string | null; handoff_ticket_id: string | null; claimed_by: string | null; handoff_requested_at: string | null; handoff_expires_at: string | null; message_count: number; last_message_at: string; last_dispatch_at: string | null; created_at: string; updated_at: string; version: number };
...
export type TaskStreamConnectionMode = "ws" | "degraded_http_polling" | "disconnected";
export type TaskStreamHealth = { protocol_version: string; connection_mode: TaskStreamConnectionMode; connected_at: string | null; last_event_at: string | null; last_disconnect_at: string | null; last_disconnect_code: number | null; last_disconnect_reason: string; reconnect_count: number; fallback_poll_count: number; upgrade_required: boolean };
export type NodeInventoryConnectionState = "connected" | "pairing_pending" | "waiting_pair" | "register_failed" | "needs_repair" | "auth_failed" | "paired_offline" | "online_unpaired";
export type NodeInventoryRecord = { node_id: string; node_kind: NodeKind; paired: boolean; online: boolean; connection_state: NodeInventoryConnectionState; status: string | null; last_heartbeat_at: string | null; updated_at: string | null; hostname: string | null; lan_ip: string | null; platform: string | null; node_version: string | null; advertised_address: string | null; last_error: string | null; base_url: string | null; max_concurrency: number | null; current_load: number | null; channel_capacity: number | null; channel_in_use: number | null; last_pairing_trace_id: string | null; last_register_result: string | null; last_register_at: string | null; last_auth_failure_at: string | null; task_stream: TaskStreamHealth };
...
export type NodeDiagnosticsRecord = { node_id: string; node_kind: NodeKind; connection_state: NodeInventoryConnectionState; last_error: string; last_pairing_trace_id: string; last_pairing_status: string; last_pairing_at: string | null; last_register_result: string; last_register_at: string | null; last_heartbeat_result: string; last_heartbeat_at: string | null; last_auth_failure_at: string | null; last_auth_decision: string; last_auth_client_host: string; last_auth_path: string; expected_token_masked: string; provided_token_masked: string; latest_task: NodeLatestTaskRecord; task_stream: TaskStreamHealth; timeline: NodeDiagnosticsEvent[] };
...
export type HumanReplyRequest = { content: string; employee_id?: string };
export type HumanReplyResponse = { ok: boolean; session: SessionRecord; message: MessageRecord | null; detail: string };
export type SessionReleaseResponse = { ok: boolean; session: SessionRecord; detail: string };
...
export type GatewaySetupConfig = {
  ...
  public_entry_greeting_message: string;
  dify_base_url: string;
  dify_api_key: string;
  builtin_model_base_url: string;
  builtin_model_api_key: string;
  builtin_model_name: string;
  builtin_model_enable_thinking: boolean;
  builtin_model_temperature: number;
  builtin_model_top_p: number;
  builtin_model_max_tokens: number;
  builtin_model_seed: number;
  builtin_model_thinking_budget: number;
  builtin_model_stop: string;
  builtin_model_enable_search: boolean;
  builtin_model_search_forced: boolean;
  builtin_model_search_strategy: string;
  builtin_model_enable_search_extension: boolean;
  builtin_model_multimodal_enabled: boolean;
  wechat_base_url: string;
  wechat_token: string;
  dispatch_mode_enabled: boolean;
};
export type WorkerNodeSetupConfig = {
  node_id: string;
  ...
  openai_seed: number;
  openai_thinking_budget: number;
  openai_stop: string;
  openai_enable_search: boolean;
  openai_search_forced: boolean;
  openai_search_strategy: string;
  openai_enable_search_extension: boolean;
  openai_multimodal_enabled: boolean;
...
export type PublicEntryProfileResponse = { enabled: boolean; base_url: string; display_name: string; qr_url: string; contact_hint: string; notes: string; greeting_message: string; access_url: string; stats: PublicEntryTicketStats };
...
export type LocalNodeChannelAssessmentStartRequest = { max_rounds: number };
export type LocalNodeChannelAssessmentApplyRequest = { strategy: "balanced" | "peak" };
export type LocalNodeConfigApplyState = "idle" | "saving" | "restarting" | "applied" | "failed";
export type LocalNodeChannelAssessmentRound = { round_index: number; max_concurrency: number; channel_capacity: number; request_count: number; success_count: number; failure_count: number; timeout_count: number; success_rate: number; average_latency_ms: number; max_latency_ms: number; stable: boolean; stop_reason: string; summary: string; first_error: string; failure_details: string[] };
export type LocalNodeChannelAssessmentResult = { status: "idle" | "running" | "blocked" | "completed" | "failed" | string; started_at: string | null; finished_at: string | null; current_channel_capacity: number; current_max_concurrency: number; recommended_channel_capacity: number | null; recommended_max_concurrency: number | null; balanced_channel_capacity: number | null; balanced_max_concurrency: number | null; summary: string; rounds: LocalNodeChannelAssessmentRound[]; risk_level: "low" | "medium" | "high" | "unknown" | string; can_start: boolean; start_blocking_reason: string; blocking_reason: string; stage: string; active_session_count: number; active_task_count: number; last_error: string };
export type LocalNodeStatusResponse = { service_name: string; state: string; service_status: string; pid: number | null; node_kind: NodeKind; config_path: string; diagnostics_path: string; install_dir: string; repair_required: boolean; repair_reason: string; venv_status: string; last_install_error: string; detail: string; service_state: string; runtime_state: string; last_register_result: string; last_register_error: string; last_register_at: string | null; config_apply_state: LocalNodeConfigApplyState; last_apply_error: string; last_apply_at: string | null; configured_model_provider: string; active_model_provider: string; inference_ready: boolean; inference_detail: string; diagnostics: Record<string, unknown>; task_stream: TaskStreamHealth; channel_assessment: LocalNodeChannelAssessmentResult; model_settings: LocalNodeModelConfig };
