...
export type LocalNodeConversationTestRequest = { provider: "current" | "openai" | "dify"; message: string };
export type LocalNodeConversationTestResponse = { ok: boolean; provider: string; configured_provider: string; config_path: string; latency_ms: number; detail: string; reply: string; usage: Record<string, unknown> };
export type WorkspaceTab = "quick_setup" | "sessions" | "connection" | "conversation_test" | "logs";
