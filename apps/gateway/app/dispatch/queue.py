...
class DispatchQueue:
    ...
    async def switch_session_target(
        ...
        *,
        ...
        if session.assigned_node_id == node_id and session.assigned_slot_id:
            detail = f"已绑定到 {session.assigned_node_id} / {session.assigned_slot_id}"
        elif session.assigned_node_id == node_id:
            detail = f"已绑定到 {session.assigned_node_id}，当前该节点暂无可分配通道。"
        else:
            detail = f"节点 {node_id} 当前不可用，未完成绑定。"
        return session, detail
    ...
    async def _ensure_slot_assignment(
        ...
        *,
        ...
        if preferred_node_id is None and routing_mode == RoutingMode.MANUAL:
            preferred_node_id = session.assigned_node_id
        if preferred_node_id:
            candidates = [candidate for candidate in candidates if candidate.node_id == preferred_node_id]
