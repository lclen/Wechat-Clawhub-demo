...
**下一步方向**：
- 中期演进：节点事件流增强能力（配置更新 / 任务取消 / 优雅关闭）
- 中期演进：员工协作闭环验证与审计链补强
- 持续优化：`trace_id` 扩展、诊断历史事件存储
- 前端统一：继续沿着单前端多角色路线收口，见 [frontend-unification-plan.md](/D:/wechat-claw-hub/docs/frontend-unification-plan.md)
...
1. **实现 diagnostics 历史事件存储**（低优先级）
   - 当前已完成“节点诊断状态持久化”，但仍缺完整历史事件仓库
   - 将 `diagnostics` 事件存储到 Redis 或数据库
   - 支持历史诊断查询、回放与分析

2. **评估是否进入去轮询阶段**（长期目标）
   - 当前 WebSocket + HTTP 降级已足够稳定
   - 后续只有在可观测性、重连与订阅模型成熟后，再考虑移除部分 HTTP 降级接口
...
1. **扩展 `trace_id` 覆盖范围**（中优先级）
   - 当前 pairing / auth / diagnostics 已部分支持
   - 下一步扩展到任务分发、结果回传、通道释放等关键链路
   - 统一日志格式，便于追踪完整调用链

2. **增强诊断事件能力**（低优先级）
   - 当前已具备诊断快照持久化
   - 下一步补完整历史事件存储、查询、聚合与告警能力
...
2. **把 Dify 版上下文隔离与节点空闲池调度合并成一个闭环**
   - 当节点持续释放空闲槽位后，网关的调度策略可以进一步向“优先选空闲节点、减少热点节点堆积”收敛

3. **验证并固化前端空闲通道展示**
   - 前端现已基于 `channel_capacity / channel_in_use` 展示空闲与占用
   - 下一步重点不再是“做出来”，而是验证 summary 推送、节点释放、节点断开后的展示一致性

下一步最值得做的事情：

1. **把 `dify_conversation_id` 做跨节点快速恢复**（中优先级）
   - 当前节点重启和跨节点迁移时，仍主要依赖本地缓存 + recent messages metadata 回填
   - 建议把最新 `dify_conversation_id` 进一步固化到更稳定、跨节点可见的真值来源

2. **把节点空闲池调度真正收口**（中优先级）
   - 节点侧空闲释放已具备基础能力
   - 下一步重点是调度策略向“优先选空闲节点、减少热点节点堆积”收敛

3. **持续验证前端空闲通道展示与 summary 实时一致性**（低优先级）
   - 不新增接口，继续复用现有 `summary/ws`
   - 重点验证节点重连、释放、删除时的前端状态是否一致
...
- ✅ 原有竞态条件已修复（详见 `docs/node-event-stream-race-condition-fix.md`）
- ⚠️ `diagnostics` 事件已接收并纳入诊断快照，但历史事件存储仍未完成
...
3. ⏳ **第三阶段：增强事件能力**（未来）
   - 添加 `diagnostics_event` 历史存储（实时诊断信息）
   - 评估是否需要显式 `channel_state_event`（当前部分状态已可由 summary 推导）
   - 添加 `config_update_event`（配置更新推送）
   - 添加 `task_cancel_event`（任务取消通知）
   - 添加 `shutdown_event`（优雅关闭通知）
...
1. **高优先级**（频繁变化，实时性要求高）
   - ✅ 会话消息（已完成）
   - ✅ 会话列表更新（已完成）
   - ✅ 节点状态变化（已主要统一到 `summary/ws`）
   - ✅ 微信状态变化（已主要统一到 `summary/ws`）

2. **中优先级**（变化较少，但影响用户体验）
   - ✅ 节点列表更新（已统一到 `summary/ws`）
   - ✅ 节点诊断更新（已有独立 WebSocket）

3. **低优先级**（进一步统一化，而非功能缺失）
   - ⏳ 单一控制台事件流 `/api/console/stream`
   - ⏳ 更细粒度按需订阅
   - ⏳ 模型配置变更推送
...
**当前进度**：现有分流式 WebSocket 体系已覆盖会话、摘要、节点诊断等关键状态；未完成的是“是否需要再统一成单一控制台事件流”。
...
- 🔄 `trace_id` 扩展到更多关键链路
- 🔄 诊断历史事件存储和分析能力增强
- 🔄 节点事件流增强能力（`config_update` / `task_cancel` / `shutdown`）
- ⏳ 控制台实时事件进一步统一化（可选，不是当前阻塞项）
