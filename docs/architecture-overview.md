...
- `role` 是显式交互状态，不是持久化恢复状态；页面刷新后只能恢复字段草稿，不能直接恢复到上一次角色
- 快速配置初始化若仍处于 `quick_setup`，默认回到“选择角色”步骤，而不是按本地缓存自动进入网关、节点或控制台配置页
- “重新配置”与“重新选角色”都视为状态机的 reset 边界；除服务端已保存配置外，前端必须清空本地角色缓存和工作区缓存
- 工作区缓存只能作为已完成部署后的导航偏好，不能在未重新选角色情况下驱动快速配置流程

前端运行态读取还需分为三层：

- 启动即读的最小状态：当前角色、网关是否在线、上次工作区、上次选中的会话与节点
- 可缓存的摘要状态：微信接入摘要、节点 summary、最近会话列表；允许先展示缓存，再后台刷新
- 必须实时拉取的状态：当前会话消息流、当前节点诊断时间线、服务重启后的最新状态；不得用本地缓存代替实时请求
